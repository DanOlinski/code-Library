...
/*
when it hits R, it's going back to previous pipe
G═A=R=P
*═╗XX
XB╝XX
*/
const input2 = [
  ['*', 0, 1], //{ index: 0, icon: 'A', matrixCoordinates: [ 0, 1 ] }
  ['═', 1, 1], //{ index: 1, icon: 'A', matrixCoordinates: [ 1, 1 ] }
  ['╗', 2, 1], //{ index: 2, icon: 'A', matrixCoordinates: [ 2, 1 ] }
  ['╝', 2, 0], //{ index: 3, icon: 'A', matrixCoordinates: [ 2, 0 ] }
  ['B', 1, 0], //{ index: 4, icon: 'A', matrixCoordinates: [ 1, 0 ] }
  ['G', 0, 2], //{ index: 5, icon: 'A', matrixCoordinates: [ 0, 2 ] }
  ['═', 1, 2], //{ index: 6, icon: 'A', matrixCoordinates: [ 1, 2 ] }
  ['A', 2, 2], //{ index: 7, icon: 'A', matrixCoordinates: [ 2, 2 ] }
  ['═', 3, 2], //{ index: 8, icon: '═', matrixCoordinates: [ 3, 2 ] }
  ['R', 4, 2], //{ index: 9, icon: 'R', matrixCoordinates: [ 4, 2 ] }
  ['═', 5, 2], //{ index: 10, icon: '═', matrixCoordinates: [ 3, 2 ] }
  ['P', 6, 2] //{ index: 11, icon: 'P', matrixCoordinates: [ 4, 2 ] }
];
...
*/
const input3 = [
  ['*', 0, 1], //{ index: 0, icon: '*', matrixCoordinates: [ 0, 1 ] }
  ['═', 1, 1], //{ index: 1, icon: '═', matrixCoordinates: [ 1, 1 ] }
  ['╗', 2, 1], //{ index: 2, icon: ''╗', matrixCoordinates: [ 2, 1 ] }
  ['╝', 2, 0], //{ index: 3, icon: '╝', matrixCoordinates: [ 2, 0 ] }
  ['B', 1, 0], //{ index: 4, icon: 'B', matrixCoordinates: [ 1, 0 ] }
  ['G', 0, 2], //{ index: 5, icon: 'G', matrixCoordinates: [ 0, 2 ] }
  ['═', 1, 2], //{ index: 6, icon: '═', matrixCoordinates: [ 1, 2 ] }
  ['A', 2, 2], //{ index: 7, icon: 'A', matrixCoordinates: [ 2, 2 ] }
  ['═', 3, 2], //{ index: 8, icon: '═', matrixCoordinates: [ 3, 2 ] }
];
...
*/
const input4 = [
  ['*', 0, 0], //{ index: 0, icon: '*', matrixCoordinates: [ 0, 1 ] }
  ['G', 0, 1], //{ index: 5, icon: 'G', matrixCoordinates: [ 0, 2 ] }
  ['═', 1, 1], //{ index: 6, icon: '═', matrixCoordinates: [ 1, 2 ] }
  ['A', 2, 1], //{ index: 7, icon: 'A', matrixCoordinates: [ 2, 2 ] }
  ['╝', 3, 1], //{ index: 7, icon: 'A', matrixCoordinates: [ 2, 2 ] }
];
...
const input5 = [
  ['*', 0, 0], //{ index: 0, icon: '*', matrixCoordinates: [ 0, 1 ] }
  ['G', 0, 1], //{ index: 5, icon: 'G', matrixCoordinates: [ 0, 2 ] }
  ['A', 1, 1], //{ index: 6, icon: '═', matrixCoordinates: [ 1, 2 ] }
  ['M', 2, 1], //{ index: 7, icon: 'A', matrixCoordinates: [ 2, 2 ] }
];
...
//input; 
//3 ifo parts, seperated by a space
//(a single array instead of a matrix), 
//input is a string containing a file path for a text file
...
const sinkSystem = (filePath) => {
  ...
  //this object contains the index of all items checked, the items are added to this object at every iteration in order to avoid the pipe system from adding duplicate letters to the final answer
  let checkedItems = {
    ...
    for (let elementIndex = 0; elementIndex < matrix.length; elementIndex++) {
      ...
      for (const adjacentOption of adjacentCoordinates) {
        ...
        if (compareCoordinates(adjacentOption, coordinate)) {
          ...
          // console.log('all possibilites', row)
          // console.log('all possibilites', elementIndex)
          // console.log(previousSource['index'])
          // console.log(previousSource['index'] === elementIndex)
  ...
  const getConnectedPipe = (matrix, pipeInfo, sourceInfo, previousSource) => {
    ...
    //at every iteration of this recursion function add the index of elements that have already been processed to prevent sinks from being added to the final answer more than once
...
console.log(sinkSystem('./data.txt'));
...
//(a single array instead of a matrix), 
//input is a string containing a file path for a text file
//fix not processing the last row
//rewrite function (check if there is anything that can be removed from the getConnected pipe)
...
const sinkSystem = (filePath) => {
    ...
    for (let row = 0; row < matrix.length; row++) {
      if (matrix[row][0] === "*") {
        ...
        const initialSource = {
          'index': row,
          'icon': matrix[row][0],
          'matrixCoordinates': [Number(matrix[row][1]), Number(matrix[row][2])]
        };
        return initialSource;
      }
    }
  ...
  const getPipeInfo = (pipesInMatrix, mappedPipes) => {
    ...
    for (let pipeInMatrix = 0; pipeInMatrix < pipesInMatrix.length; pipeInMatrix++) {
      ...
      let pipeIndex = pipesInMatrix[pipeInMatrix]['matrixIndex'];
      let pipeIconMatrix = matrix[pipesInMatrix[pipeInMatrix]['matrixIndex']][0];
      let pipeCoordinatesInMatrix = [Number(matrix[pipesInMatrix[pipeInMatrix]['matrixIndex']][1]), Number(matrix[pipesInMatrix[pipeInMatrix]['matrixIndex']][2])];
  ...
  const checkAllAdjacentElements = (matrix, mainElementIndex, previousSource) => {
    ...
    const north = [Number(matrix[mainElementIndex][1]), Number(matrix[mainElementIndex][2]) + 1];
    const south = [Number(matrix[mainElementIndex][1]), Number(matrix[mainElementIndex][2]) - 1];
    const east = [Number(matrix[mainElementIndex][1]) + 1, Number(matrix[mainElementIndex][2])];
    const west = [Number(matrix[mainElementIndex][1]) - 1, Number(matrix[mainElementIndex][2])];
    ...
    for (let elementIndex = 0; elementIndex < matrix.length; elementIndex++) {
      const row = matrix[elementIndex];
      const coordinate = [Number(row[1]), Number(row[2])];
          ...
          if (compareCoordinates(outflow, coordinatesOfMatrixItems)) {
            ...
            if (checkIfIconIsALetter(adjacentIcon) && !(checkedItems['index' + `${index}`])) {
              const adjacentElement = {
                index: index,
                icon: adjacentIcon,
                matrixCoordinates: [Number(matrix[index][1]), Number(matrix[index][2])]
              };
